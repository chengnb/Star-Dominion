...
│   └── game/
│       ├── constants.js         # 游戏常量 (区块/迷雾/行星/飞船/战斗)
│       ├── SpatialGrid.js       # 空间分区网格 (Map 实现，无限坐标)
│       └── GameWorld.js         # 游戏核心 — 区块生成/战斗/迷雾/持久化/通知
├── client/                      # 前端 — 无框架，原生 JS + Canvas
│   ├── index.html               # 单页面: 登录界面 + 游戏画布 + 快捷面板 + UI
│   ├── css/style.css            # 全部样式 (含系统消息/快捷面板/退出按钮)
│   └── js/
...
### 3. 战争迷雾 (Fog of War)
`_computeFogVisibility(socketId)` 计算玩家可见的行星 ID 集合：
- 玩家己方行星 **始终可见**
- 己方行星周围 `FOG_RADIUS`(700单位) 内的行星可见
- 己方舰队周围 700 单位内的行星可见（舰队航行途中照亮航线）
- 发生变更的己方脏行星始终可见
- `getVisibleState` 和 `getFullState` 都使用迷雾过滤
- 己方舰队全量发送（不限视口），敌方舰队按视口显示

**客户端迷雾处理**: `onUpdate` 追踪激活行星 ID，不在更新中的行星从本地 Map 移除（己方行星除外）。
...
### 6. 用户缓存 (userCache)
`GameWorld.userCache`: `Map<userId, {username, color}>` — 启动时从 users 表加载，新玩家加入时追加。颜色由 `userId % palette.length` 决定，保证一致性。用于离线玩家行星显示和排行榜。

### 7. 状态同步策略
- **首次连接**: `game:init` 发送状态（迷雾过滤后的行星/舰队/在线玩家/ownerCache）
- **每 tick**: `game:update` 发送迷雾可见行星 + 己方全量舰队 + 视口内敌方舰队，`volatile.emit`
...
### 8. 事件委托 + DOM 最小更新 (客户端)
行星面板和快捷选择面板使用事件委托，**不在每个 tick 重新绑定或设置 innerHTML**：
- **行星面板**: 选中行星变化时完整重建(`_updatePlanetPanel`)，每 tick 仅刷新文本和按钮状态(`_refreshPlanetPanel`)
- **快捷面板**: 用 `_lastQuickIds` 指纹检测行星增减，仅在变化时重建 DOM，每 tick 仅更新 `textContent` 和 `classList`
- 所有事件监听器在构造函数中绑定一次
...
| 战争迷雾 | `server/game/GameWorld.js:_computeFogVisibility` |
| 己方行星快捷选择面板 | `client/js/Game.js:_refreshQuickSelect` |
...
| 舰队悬浮提示 (拥有者/舰船构成/目的地) | `client/js/Game.js` render hover |
...
| 区块参数 (尺寸/密度/迷雾半径) | `constants.js` CHUNK_SIZE / PLANETS_PER_CHUNK / FOG_RADIUS |
...
| 修改 UI/渲染/迷雾 | `client/js/Game.js`；CSS `client/css/style.css` |
    ...
    → world.enqueueCommand(socketId, cmd)
      → [下一 tick] world._processCommands()
        → world._handleXxx(cmd)  // 修改状态，_markPlanetDirty(id)
          → [同一 tick] world._computeFogVisibility(socketId) + getVisibleState
            → socket.volatile.emit('game:update', state)
              → 客户端 Game.onUpdate(data) → 更新 planets/fleets Map → 移除过期行星 → 渲染
```
...
```
每 tick → _computeFogVisibility(socketId)
  → 收集己方行星列表 (ownerId === socketId)
  → 对每个己方行星: spatialGrid.query(planet.x, planet.y, FOG_RADIUS)
  → 对每个己方舰队: spatialGrid.query(fleet.x, fleet.y, FOG_RADIUS)
  → 追加己方脏行星 (被攻击等)
  → 返回可见行星 ID 集合
```
...
requestAnimationFrame 循环:
  ...
  4. 绘制己方行星连线
  5. 绘制迷雾内行星 (渐变球体 + 拥有者光环 + 防御标记 + 名称标签)
  6. 绘制舰队航线虚线 + 舰队 (三角箭头 + 数量标签)
  7. 绘制选中行星指示器 (虚线脉冲光环)
  8. 行星/舰队悬浮提示框
  9. HUD 独立 DOM 层 (文本增量更新，不设 innerHTML)
  10. 快捷面板 DOM 更新 (指纹检测 + textContent)
  11. 小地图 (以镜头为中心的动态范围)
```
...
2. **不在每 tick 修改 DOM innerHTML**: 用 textContent 更新文本，用 classList 改状态。快捷面板用指纹检测 + 增量更新。
...
8. **迷雾客户端同步**: `onUpdate` 移除不在更新中的非己方行星，防止过期数据残留。
9. **快捷面板指纹**: `_lastQuickIds` 检测行星 ID 集合变化，仅在增减时重建 HTML，其余 tick 只更新 textContent。
